...
const mainVertex = tgpu.vertexFn({
  ...
  return {
    outPos: vec4f(result.vertexPosition.mul(result.w), 0, result.w),
...
const mainFragment = tgpu.fragmentFn({
  ...
  color = color.mul((0.8 + 0.2 * smoothstep(1, 0.5, uv.y)));
  if (frontFacing) {
...
const circlesVertex = tgpu.vertexFn({
  ...
  return {
    outPos: vec4f(vertex.position.add(unit.mul(vertex.radius)), 0, 1),
  };
});
