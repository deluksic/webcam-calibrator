...
const segmentAlternate = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const segmentStretch = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const side = segmentSide.$[vertexIndex];
...
const segmentContainsAnotherEnd = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const side = segmentSide.$[vertexIndex];
  return LineControlPoint({
...
const caseVShapeSmall = testCaseShell((vertexIndex, t) => {
  'use gpu';
...
const caseVShapeBig = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const side = clamp(f32(vertexIndex) - 2, -1, 1);
  const isMiddle = side === 0;
  return LineControlPoint({
...
const halfCircle = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const halfCircleThin = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const bending = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const animateWidth = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const perlinTraces = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const bars = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const arms = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const s = sin(time);
  const c = cos(time);
  const r = 0.25;
...
const armsSmall = testCaseShell((vertexIndex, time) => {
  'use gpu';
...
const armsBig = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const result = arms(vertexIndex, time);
  return LineControlPoint({
    position: result.position,
...
const armsRotating = testCaseShell((vertexIndex, time) => {
  'use gpu';
  const s = sin(time);
  const c = cos(time);
  const r = 0.25;
...
const flyingSquares = testCaseShell((vertexIndex, time) => {
  'use gpu';
  ...
  const transformedPoint = vec2f(x, y).add(rotate.mul((point.mul(r))));
  return LineControlPoint({
...
const spring = testCaseShell((vertexIndex, time) => {
  'use gpu';
