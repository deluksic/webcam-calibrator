...
const { min, max, sqrt, log, cos, sin, PI } = Math
...
export function applyAdditiveGaussian01(intensity: Float32Array, sigma: number, seed: number): void {
  ...
  const nextGaussian = (): number => {
    ...
    const r = sqrt(-2 * log(max(1e-15, u1)))
    const z0 = r * cos(2 * PI * u2)
    const z1 = r * sin(2 * PI * u2)
    spare = z1
    return z0
  }
  for (let i = 0; i < intensity.length; i++) {
    intensity[i] = min(1, max(0, intensity[i]! + nextGaussian() * sigma))
  }
}
