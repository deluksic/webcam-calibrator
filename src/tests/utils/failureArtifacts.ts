...
export function failureDirForTask(testFileUrl: string, task: FailureTask): string {
  const fileLabel = sanitizeSegment(testFileUrl.replace(/^.*\/src\//, 'src/').replace(/\.test\.ts$/, ''))
...
export function attachFailureArtifacts(testFileUrl: string, write: (dir: string) => void): void {
  onTestFailed(({ task }) => {
    const dir = failureDirForTask(testFileUrl, {
      name: String(task.name ?? task.id ?? 'test'),
    })
    try {
      write(dir)
    } catch (e) {
...
export function writeSobelMagPng(path: string, width: number, height: number, sobel: Float32Array): void {
  let m = 0
...
export function writeCellLegendPng(path: string, decoded: TagPattern, truth: TagPattern, scale: number): void {
  const cw = 6 * scale
  const ch = 6 * scale
