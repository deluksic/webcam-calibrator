...
describe('parseFoundErrorCount', () => {
  ...
  it('parses tsc watch timestamp prefix before Found', () => {
    expect(parseFoundErrorCount('9:06:31 PM - Found 17 errors. Watching for file changes.')).toBe(17)
  })
...
Found 0 errors. Watching for file changes.`
    expect(await resolveTscWatcherStatus(false, stalePassLog)).toEqual({
      status: 'stopped',
    })
...
✓ built in 0.01s`
    expect(await resolveBuildWatcherStatus(false, staleBuildLog)).toEqual({
      status: 'stopped',
    })
  })
...
✓ 1 modules transformed.`
    expect(await resolveBuildWatcherStatus(true, blog)).toEqual({
      status: 'pass',
    })
  })
})
...
describe('checkResultsNeedFailureExit', () => {
  it('is true when either watcher failed', () => {
    expect(checkResultsNeedFailureExit({ status: 'fail', content: 'x' }, { status: 'pass' })).toBe(true)
    expect(checkResultsNeedFailureExit({ status: 'pass' }, { status: 'fail', content: 'y' })).toBe(true)
  })
      ...
      checkResultsNeedFailureExit(
        {
          status: 'building',
          content: '',
          snippet: { kind: 'last', shown: 0, total: 0 },
        },
        { status: 'pass' },
      ),
    ).toBe(false)
    expect(checkResultsNeedFailureExit({ status: 'stopped' }, { status: 'stopped' })).toBe(false)
  })
})
