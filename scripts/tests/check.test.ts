...
describe("parseFoundErrorCount", () => {
  ...
  it("parses tsc watch timestamp prefix before Found", () => {
    expect(parseFoundErrorCount("9:06:31 PM - Found 17 errors. Watching for file changes.")).toBe(
      17,
    );
  });
...
describe("checkResultsNeedFailureExit", () => {
  it("is true when either watcher failed", () => {
    expect(checkResultsNeedFailureExit({ status: "fail", content: "x" }, { status: "pass" })).toBe(
      true,
    );
    expect(checkResultsNeedFailureExit({ status: "pass" }, { status: "fail", content: "y" })).toBe(
      true,
    );
  });
