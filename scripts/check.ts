...
export function checkResultsNeedFailureExit(tsc: ParseCheckResult, build: ParseCheckResult): boolean {
...
export async function resolveTscWatcherStatus(processRunning: boolean, logContent: string): Promise<ParseCheckResult> {
  if (!processRunning) {
...
function buildRunLooksComplete(sanitizedLines: string[]): boolean {
  return sanitizedLines.some(
    (l) => l.includes('modules transformed') || /\bbuilt in\s+[\d.]+/i.test(l) || /\b✓\s*built in\b/i.test(l),
  )
}
...
export async function parseBuildLog(content: string): Promise<ParseCheckResult> {
  ...
  if (buildRunLooksFailed(sanitizedRun)) {
    ...
    const displayLines = fullLen > SNIPPET_MAX_LINES ? lastRunOutput.slice(-SNIPPET_MAX_LINES) : lastRunOutput
    const body = displayLines.join('\n').trim()
    return {
      status: 'fail',
      snippet: {
...
export async function runCheck(): Promise<void> {
  ...
  const build =
    buildRead.kind === 'missing'
      ...
      : await resolveBuildWatcherStatus(buildRunning, buildRead.kind === 'ok' ? buildRead.content : '')
